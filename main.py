...
try:
    ...
    RVCInference = None
    # NOT: Sadece str(_e) değil, tipini de saklıyoruz. Örn. bir
    # NameError, "rvc-python kurulu değil" ile karıştırılabilecek
    # şekilde belirsiz bir mesaj verebiliyordu (bkz. pyi_rth_site_builtins.py).
    RVC_IMPORT_ERROR = f"{type(_e).__name__}: {_e}"
    traceback.print_exc()  # konsola/log dosyasına tam iz düşümü yaz
